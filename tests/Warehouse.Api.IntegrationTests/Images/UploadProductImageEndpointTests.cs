...
/// <summary>
/// POST /api/products/{id}/image — binary upload.
/// Asserts the HTTP contract, the exact bytes that land on disk, and what does (and does
...
public class UploadProductImageEndpointTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    /// <summary>Creates a real product through the API and returns its id.</summary>
    ...
    {
        // The endpoint streams the upload straight to disk with no re-encoding, so the stored
        // file must match the payload exactly.
        var id = await CreateProductAsync();
    ...
    public async Task POST_Image_FileIsNamedAfterTheProductId()
    {
        ...
        // The uploaded filename is discarded; only its extension survives.
        File.Exists(expectedPath).Should().BeTrue();
    ...
    public async Task POST_Image_SecondUploadOverwritesTheFirst()
    {
        ...
        // FileMode.Create truncates, so no bytes of the larger first upload survive.
        var stored = await File.ReadAllBytesAsync(expectedPath);
    ...
    {
        // GAP: the schema has a Productimages table with a FK to Products, but this endpoint
        // writes only to local disk. Nothing in src/ ever inserts a Productimages row, so the
        // upload leaves no database trace at all -- the image cannot be discovered by query,
        // only by guessing the {productId}.{ext} filename convention.
        var id = await CreateProductAsync();
        TrackPath(id, ".jpg");
        using var form = BuildForm("photo.jpg", "image/jpeg", new byte[] { 1, 2, 3, 4 });
    ...
    {
        // GAP: "image processing" is extension-matching only. There is no magic-byte check,
        // no decode, no re-encode -- so arbitrary bytes are accepted and stored under a .jpg
        // name purely because the filename ended in .jpg.
        var id = await CreateProductAsync();
        var expectedPath = TrackPath(id, ".jpg");
    ...
    {
        // Only the filename extension is inspected; the multipart Content-Type is not checked.
        var id = await CreateProductAsync();
        var expectedPath = TrackPath(id, ".png");
