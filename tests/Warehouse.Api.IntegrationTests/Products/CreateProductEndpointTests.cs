...
/// <summary>
/// POST /api/products — full product initialisation flow.
/// Asserts the HTTP contract (status, headers, body) and the persisted side effect
/// by reading the row back out of the application's own database.
/// </summary>
public class CreateProductEndpointTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    // Unique per call so tests in this class cannot collide on the SKU uniqueness rule.
    ...
    public async Task POST_ValidProduct_SetsLocationHeaderToTheNewResource()
    {
        ...
        // Note the casing: CreatedAtAction builds the URL from the [controller] route token,
        // so the header reads "/api/Products/..." with a capital P, not the lowercase
        // "/api/products" that every hand-written call in this codebase uses. Routing is
        // case-insensitive on the way back in, so both work -- but a client doing a literal
        // string comparison against the Location header would be surprised.
    ...
    {
        // GAP: the create flow copies SupplierName as a free-text string and never resolves it
        // to a Supplier row, so SupplierId stays null even though "Acme Corp" exists. The
        // product is only associated with its supplier by a denormalised name.
        var sku = NewSku();
        var request = ValidRequest(sku);
