...
///
/// IMPORTANT: despite the HTTP verb this is a SOFT delete. The handler calls
/// Product.Archive() and persists IsArchived = true; no row is ever removed and no related
/// data is cleaned up. These tests assert that real behaviour rather than the teardown /
/// cascading-cleanup semantics the verb implies, and mark each cleanup that does not happen.
/// </summary>
public class DeleteProductEndpointTests : IClassFixture<CustomWebApplicationFactory>, IDisposable
{
    ...
    // ---------- persisted side effects: what actually happens ----------
    ...
    // ---------- observable effects through the API ----------
    ...
    // ---------- cascading cleanup: what does NOT happen ----------

    [Fact]
    public async Task DELETE_Product_LeavesUploadedImageFileOnDisk()
    {
        // GAP: no filesystem cleanup. The uploaded image outlives the "deleted" product and
        // nothing will ever remove it -- an unbounded leak of orphaned files.
        var id = await CreateProductAsync();
    ...
    {
        // GAP: no cascading delete of related rows. Archiving touches only the Products row.
        var id = await CreateProductAsync();
    ...
    // ---------- negative and idempotency cases ----------
    ...
    {
        // The {id:guid} route constraint rejects non-GUIDs before the action runs.
    ...
    {
        // Archive() is unconditional, so deleting twice succeeds rather than 404/409-ing.
        // Idempotent from the caller's point of view, but it also means the archive
        // timestamp is overwritten on every repeat call.
        var id = await CreateProductAsync();
    ...
    {
        // GAP: the image endpoint only checks that the product exists, not whether it is
